...
export default function PersonDetail() {
      ...
      <div className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-100 via-slate-50 to-slate-200 p-6 md:p-12 font-sans flex justify-center">
        <div className="w-full max-w-5xl space-y-6">
          <div className="bg-slate-900 rounded-3xl p-8 border border-slate-800 shadow-2xl animate-pulse">
            <div className="w-24 h-4 bg-slate-800 rounded mb-6"></div> 
            <div className="w-1/3 h-10 bg-slate-800 rounded-xl mb-4"></div> 
            {/* SKELETON: flex-wrap eklendi */}
  ...
  if (!person) {
    return (
      <div className="text-center py-20">
        ...
        {/* Hata ekranındaki butonu da panel'e yönlendirdik */}
        <button onClick={() => navigate('/panel')} className="mt-4 text-blue-500 underline font-bold">Listeye Dön</button>
      </div>
    );
  }
  ...
  return (
    <div className={S.wrapper}>
      ...
      {/* Menü: Form Sekmeleri - flex-wrap eklendi */}
      <nav className={`${S.tabContainer} flex flex-wrap gap-2 mb-6`}>
        {tabs.map(tab => {
          ...
          return (
            <button 
              ...
              className={`
                ${S.tabBtnBase} flex items-center gap-2 px-4 py-2 rounded-xl transition-all font-bold text-sm border
                ${activeTab === tab 
                ...
                activeRecords
                  ...
                  .map((record, index) => (
                    <div key={index} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                      <div className="border-b border-slate-100 pb-3 mb-3 flex flex-wrap justify-between items-center gap-2">
